...
namespace sx::cfg {
    ...
    void process_tenant_config_line (std::string const &line, vec_tenants& retvar) {
        ...
        std::for_each(params.begin(), params.end(), [] (std::string &p) {
            p = std::regex_replace(p, std::regex("\\s+"), "");
        });
        ...
        if (params.size() >= 3) {
            ...
            retvar.emplace_back(static_cast<unsigned int>(index), params[1], params[2], ipv6);
        }
    }


    std::optional<unsigned int> find_tenant(vec_tenants const& v, std::string const& name) {
        for(auto const& ten: v) {
            if(ten.name == name) {
                return ten.index;
            }
        }
