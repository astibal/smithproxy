...
SessionList::SessionList(std::size_t workers, text_renderer renderer)
    : version_(next_version()), kind_(kind::text), text_renderer_(std::move(renderer)),
      text_fragments_(workers), remaining_(workers) {}
...
SessionList::SessionList(std::size_t workers, json_renderer renderer)
    : version_(next_version()), kind_(kind::json), json_renderer_(std::move(renderer)),
      json_fragments_(workers, nlohmann::json::array()), remaining_(workers) {}
...
void for_each_worker(List const& listeners, char const* origin, Callback&& callback) {
...
void all_session_workers(Callback&& callback) {
    auto const& instance = SmithProxy::instance();
    for_each_worker(instance.plain_proxies, "plain acceptor", callback);
    for_each_worker(instance.ssl_proxies, "tls acceptor", callback);
    for_each_worker(instance.udp_proxies, "udp receiver", callback);
    for_each_worker(instance.dtls_proxies, "dtls receiver", callback);
    for_each_worker(instance.socks_proxies, "socks acceptor", callback);
    for_each_worker(instance.socks_udp_proxies, "socks receiver", callback);
    for_each_worker(instance.redir_plain_proxies, "plain redirect acceptor", callback);
    for_each_worker(instance.redir_udp_proxies, "dns redirect receiver", callback);
    for_each_worker(instance.redir_ssl_proxies, "tls redirect acceptor", callback);
}
} // namespace

std::size_t session_list_worker_count() {
    std::size_t count = 0;
    all_session_workers([&count](auto&, char const*) { ++count; });
    return count;
}

void dispatch_session_list(std::shared_ptr<SessionList> const& request) {
    std::size_t slot = 0;
    all_session_workers([&](auto& worker, char const* origin) {
        worker.enqueue_session_list(request, slot++, origin);
    });
}
