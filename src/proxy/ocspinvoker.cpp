...
std::unique_ptr<AsyncOcspInvoker> AsyncOcspInvoker::invoke(MitmProxy& proxy) {

    if(! proxy.rs().empty()) {
        auto mh = proxy.first_right();
        if(auto *scom = dynamic_cast<SSLCom *>(mh->com()) ; scom) {

            if(scom->target_cert() && scom->target_issuer()) {

                auto new_caller = std::make_unique<AsyncOcspInvoker>(proxy);

                using std::placeholders::_1;
                new_caller->ocsp_ = std::make_unique<inet::ocsp::AsyncOCSP>(
                        scom->target_cert(),
                        scom->target_issuer(),
                        mh,
